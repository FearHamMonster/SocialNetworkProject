...
def main():
    ...
    parser.add_argument('--graph-type', type=int, default=0, help="if 0, a Holme-Kim graph is gonna be used, if 1 a Erdos-Reniy graph is gonna be used")
    ...
    parser.add_argument('--QI-attributes', type=int, default=6, help="number of QI attributes (must be between 1 and 6)")
    ...
    counter = 0
    QI_attr = []
    for attr in list(attributes_type.keys()):
        if counter >= args.QI_attributes:
            attributes_type.pop(attr)
        else:
            QI_attr.append(attr)
        counter += 1
    
    if args.graph_type == 0:
        G = graphgen.generate_powerlaw_graph(args.p, args.avarage_degree, args.n, from_dataset=args.real_dataset, seed=args.seed, QI_attributes=QI_attr)
    elif args.graph_type == 1:
        G = graphgen.generate_ErdosRenyi_graph(args.n, args.avarage_degree, from_dataset=args.real_dataset, random_values=args.random_values, seed=args.seed, QI_attributes=QI_attr)
    else:
    ...
    if not args.print:
        with open(args.csv, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([args.QI_attributes, avarage_time])  
    else:
        print("with", args.QI_attributes, "attributes, avarage time =", avarage_time, "s")
